...
interface PatientDashboardProps {
  /** Omitted for the signed-in patient's own dashboard. */
  targetUserId?: string;
}

export default function PatientDashboard({ targetUserId }: PatientDashboardProps) {
  const router = useRouter();
  const adminView = Boolean(targetUserId);
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    const adminCheck = adminView
      ? supabase.from('profiles').select('is_admin').eq('id', user.id).single()
      : Promise.resolve({ data: { is_admin: false }, error: null });

    // These reads are independent after authentication, so start them together.
    // RLS still makes the session request self-only unless the caller is admin.
    const [adminResult, profileResult, sessionResult] = await Promise.all([
      adminCheck,
      supabase.from('profiles').select('name, email').eq('id', viewedUserId).single(),
      supabase
    ...
    if (adminView && (adminResult.error || !adminResult.data?.is_admin)) {
      router.replace('/');
      return;
    }
    ...
    if (profileError || !profile) {
      setError(adminView ? 'We could not find this patient.' : 'We could not load your profile.');
      setLoading(false);
      return;
    }

    setUserName(profile.name || profile.email.split('@')[0] || (adminView ? 'Patient' : 'there'));
    ...
    const { data: sessions, error: fetchError } = sessionResult;

    if (fetchError) {
      setError(adminView ? "We could not load this patient's sessions." : 'We could not load your sessions.');
      setLoading(false);
      return;
    }
    ...
    <div className="dash-page">
      ...
      <div className="dash-wrap">
        <header className="dash-head">
          ...
          {adminView ? (
            <nav className="dash-actions" aria-label="Patient dashboard actions">
              <Link href="/admin/patients" className="pill-btn pill-btn-outline">
                Back to patients
              </Link>
              <Link href={`/admin/users/${targetUserId}`} className="pill-btn pill-btn-primary">
                Edit stars
              </Link>
            </nav>
          ) : (
          ...
          <>
            ...
            <div className="dash-charts">
              <section className="dash-panel animate-fadeInUp stagger-5">
                ...
                {empty ? (
                  <Empty
                    text={adminView ? 'No completed sessions in this period' : 'Complete a session and your line starts here'}
                  />
                ) : (
              ...
              <section className="dash-panel animate-fadeInUp stagger-6">
                ...
                {empty ? (
                  <Empty text={adminView ? 'No sessions in this period' : 'Your first session plants the first stem'} />
                ) : (
                  <ResponsiveContainer width="100%" height={250}>
...
function DashboardSkeleton({ adminView }: { adminView: boolean }) {
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg)', padding: 'var(--space-6)' }}>
      <div style={{ maxWidth: '1120px', margin: '0 auto' }}>
        ...
        <span className="sr-only">{adminView ? 'Loading patient dashboard' : 'Loading your progress'}</span>
      </div>
    </div>
  );
}
